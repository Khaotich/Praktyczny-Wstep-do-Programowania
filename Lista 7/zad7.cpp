...
float vy[N] = { 0.5, 0.6, 0.7, 0.8};

Color colors[N] = { Color::Green, Color::Red, Color::Yellow, Color::Blue, Color::Magenta };
...
int main()
{
    ...
    while (window.isOpen())
    {
        for (int i = 0; i < N; i++)
        {
            ...
            if (shapes[i].getPosition().x + r[i] * 2 > width || shapes[i].getPosition().x < 0) vx[i] *= -1;
            if (shapes[i].getPosition().y + r[i] * 2 > height || shapes[i].getPosition().y < 0) vy[i] *= -1;
